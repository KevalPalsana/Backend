...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await FededgeUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await FededgeUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return FededgeUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await FededgeUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await FededgeUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return FededgeUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "FededgeUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "FededgeUser not found");
  ...
  console.log("Filtered FededgeUser for Update:", user);
  const updatedUser = await FededgeUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginAdmin  = async(username,password) => {
    console.log(`username is ${username} passwod ${password}`)
  ...
  const getReferralPrice = async (userId) => {
    const user = await FededgeUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'FededgeUser not found');
    }
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await FededgeUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'FededgeUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await FededgeUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await FededgeUser.create({
        mobileNumber,
      });
    }
