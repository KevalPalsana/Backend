...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await ChhapiaUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await ChhapiaUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return ChhapiaUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await ChhapiaUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await ChhapiaUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return ChhapiaUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "ChhapiaUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "ChhapiaUser not found");
  ...
  console.log("Filtered ChhapiaUser for Update:", user);
  const updatedUser = await ChhapiaUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginUser  = async(email,password) => {
  ...
  const loginAdmin  = async(email,password) => {
    ...
    if (!email) throw new ApiError(httpStatus.BAD_REQUEST, 'Email is required!');
    if (!password) throw new ApiError(httpStatus.BAD_REQUEST, 'password is required!');
    const user = await ChhapiaUser.findOne({ email });
  ...
  const getReferralPrice = async (userId) => {
    const user = await ChhapiaUser.findById(userId);
    if (!user) {  
      throw new ApiError(httpStatus.BAD_REQUEST, 'ChhapiaUser not found');
    }
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await ChhapiaUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'ChhapiaUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await ChhapiaUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await ChhapiaUser.create({
        mobileNumber,
      });
    }
