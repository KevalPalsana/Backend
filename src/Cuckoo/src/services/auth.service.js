...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await CuckooUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await CuckooUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return CuckooUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await CuckooUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await CuckooUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return CuckooUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "CuckooUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "CuckooUser not found");
  ...
  console.log("Filtered CuckooUser for Update:", user);
  const updatedUser = await CuckooUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginAdmin  = async(email,password) => {
    ...
    if (!email) throw new ApiError(httpStatus.BAD_REQUEST, 'Email is required!');
    if (!password) throw new ApiError(httpStatus.BAD_REQUEST, 'password is required!');
    const user = await CuckooUser.findOne({ email });
  ...
  const getReferralPrice = async (userId) => {
    const user = await CuckooUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'CuckooUser not found');
    }
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await CuckooUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'CuckooUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await CuckooUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await CuckooUser.create({
        mobileNumber,
      });
    }
