...
import VinayakToken from '../models/token.model.js';

const { sign, verify } = jwt;
/**
 * Generate token
 * @param {VinayakUser} user
 ...
 * @returns {Promise<VinayakToken>}
 */
const saveToken = async (token, userId, expires, type, blacklisted = false) => {
  const tokenDoc = await VinayakToken.create({
    token,
    user: userId,
...
const verifyToken = async (token, type) => {
  ...
  const tokenDoc = await VinayakToken.findOne({ token, type, user: payload.sub, blacklisted: false });
  if (!tokenDoc) {
    throw new Error('VinayakToken not found');
  }
  return tokenDoc;
};
