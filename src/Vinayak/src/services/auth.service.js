...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await VinayakUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await VinayakUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return VinayakUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await VinayakUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await VinayakUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return VinayakUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "VinayakUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "VinayakUser not found");
  ...
  console.log("Filtered VinayakUser for Update:", user);
  const updatedUser = await VinayakUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginUser  = async(email,password) => {
  ...
  const loginAdmin  = async(email,password) => {
    ...
    if (!email) throw new ApiError(httpStatus.BAD_REQUEST, 'Email is required!');
    if (!password) throw new ApiError(httpStatus.BAD_REQUEST, 'password is required!');
    const user = await VinayakUser.findOne({ email });
  ...
  const getReferralPrice = async (userId) => {
    const user = await VinayakUser.findById(userId);
    if (!user) {
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await VinayakUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'VinayakUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await VinayakUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await VinayakUser.create({
        mobileNumber,
      });
    }
