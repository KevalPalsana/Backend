...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await KKUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await KKUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return KKUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await KKUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await KKUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return KKUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "KKUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "KKUser not found");
  ...
  console.log("Filtered KKUser for Update:", user);
  const updatedUser = await KKUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginAdmin  = async(name,password) => {
    console.log(`name is ${name} passwod ${password}`)
    if (!name) throw new ApiError(httpStatus.BAD_REQUEST, 'Name is required!');
    if (!password) throw new ApiError(httpStatus.BAD_REQUEST, 'password is required!');
    const user = await KKUser.findOne({ name });
  ...
  const getReferralPrice = async (userId) => {
    const user = await KKUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'KKUser not found');
    }
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await KKUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'KKUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await KKUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await KKUser.create({
        mobileNumber,
      });
    }
