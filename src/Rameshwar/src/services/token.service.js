...
import RameshwarToken  from '../models/token.model.js';
 ...
 * @returns {Promise<RameshwarToken>}
 */
const saveToken = async (token, userId, expires, type, blacklisted = false) => {
  const tokenDoc = await RameshwarToken.create({
    token,
    user: userId,
...
const verifyToken = async (token, type) => {
  ...
  const tokenDoc = await RameshwarToken.findOne({ token, type, user: payload.sub, blacklisted: false });
  if (!tokenDoc) {
    throw new Error('RameshwarToken not found');
  }
  return tokenDoc;
};
