...
import SPJToken  from '../models/token.model.js';
 ...
 * @returns {Promise<SPJToken>}
 */
const saveToken = async (token, userId, expires, type, blacklisted = false) => {
  const tokenDoc = await SPJToken.create({
    token,
    user: userId,
...
const verifyToken = async (token, type) => {
  ...
  const tokenDoc = await SPJToken.findOne({ token, type, user: payload.sub, blacklisted: false });
  if (!tokenDoc) {
    throw new Error('SPJToken not found');
  }
  return tokenDoc;
};
