...
import { IconUser } from '../models/user.model.js';
...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await IconUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await IconUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return IconUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await IconUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await IconUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return IconUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "IconUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "IconUser not found");
  ...
  console.log("Filtered IconUser for Update:", user);
  const updatedUser = await IconUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginUser  = async(email,password) => {
  ...
  const loginAdmin  = async(email,password) => {
    ...
    if (!email) throw new ApiError(httpStatus.BAD_REQUEST, 'Email is required!');
    if (!password) throw new ApiError(httpStatus.BAD_REQUEST, 'password is required!');
    const user = await IconUser.findOne({ email });
  ...
  const getReferralPrice = async (userId) => {
    const user = await IconUser.findById(userId);
    if (!user) {  
      throw new ApiError(httpStatus.BAD_REQUEST, 'IconUser not found');
    }
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await IconUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'IconUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await IconUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await IconUser.create({
        mobileNumber,
      });
    }
