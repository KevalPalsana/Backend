...
const createNewUser = async (userBody = {}) => {
  ...
  while (!isUnique) {
    ...
    const existingCode = await InfiUser.findOne({ referralCode });
    if (!existingCode) {
      isUnique = true;
    }
  }
  ...
  if (userBody.referredBy) {
    const referrer = await InfiUser.findOne({ referralCode: userBody.referredBy });
    if (!referrer)
  ...
  return InfiUser.create(defaultUser);
};


 const findUserByMobileNumber = async (mobileNumber) => {
  return await InfiUser.findOne({ mobileNumber }); 
};
...
const createAdmin = async (userBody) => {
  ...
  const emailExist = await InfiUser.findOne({ email: normalizedEmail });
  if (emailExist) {
  ...
  return InfiUser.create(newUser);
};


 const updateUser = async (userId, user) => {
  if (!userId) throw new ApiError(httpStatus.BAD_REQUEST, "InfiUser ID is required!");

  const userNotExist = await userService.getUserById(userId); // Ensure this is awaited
  if (!userNotExist) throw new ApiError(httpStatus.NOT_FOUND, "InfiUser not found");
  ...
  console.log("Filtered InfiUser for Update:", user);
  const updatedUser = await InfiUser.findByIdAndUpdate(userId, user, { new: true });
  return updatedUser;
};
...
const loginUser  = async(email,password) => {
  ...
  const loginAdmin  = async(email,password) => {
    ...
    if (!email) throw new ApiError(httpStatus.BAD_REQUEST, 'Email is required!');
    if (!password) throw new ApiError(httpStatus.BAD_REQUEST, 'password is required!');
    const user = await InfiUser.findOne({ email });
  ...
  const getReferralPrice = async (userId) => {
    const user = await InfiUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'InfiUser not found');
    }
  ...
  const updateReferPrice = async (userId, amount) => {
    const user = await InfiUser.findById(userId);
    if (!user) {
      throw new ApiError(httpStatus.NOT_FOUND, 'InfiUser not found');
    }
  ...
  const verifyOtp = async (mobileNumber, inputOtp) => {
    ...
    let user = await InfiUser.findOne({ mobileNumber });
  
    if (!user) {
      user = await InfiUser.create({
        mobileNumber,
      });
    }
